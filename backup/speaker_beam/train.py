...
class SEBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        if stage != sb.Stage.TEST:
            ...
            if self.model_name == "SB_ver1":
                ...
                raise ValueError("compute forward not implemented correctly")
    ...
    def compute_objectives(self, predictions, batch, stage):
        if stage in [sb.Stage.TRAIN, sb.Stage.VALID]:
            ...
            loss_recons = sb.nnet.losses.mse_loss(
                ...
                batch.IBM.data.to(self.device).permute(0, 2, 1)
            )
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            if epoch % 10 == 5:
                masked_list, tag_list = self.test()
                for i in range(len(masked_list)):
                    self.train_logger_tfboard.writer.add_audio(
                        tag_list[i], masked_list[i] / np.abs(masked_list[i]).max(), epoch,
                        sample_rate=16000
                    )
            ...
            with open(flist, "r") as f:
                for line in f.readlines():
                    ...
                    sep_MPDR_SFT = segmentation(
                        MultiSTFT(sep_MPDR, n_fft=hparams["n_fft"]).contiguous(),
                    ...
                    noisy_SFT = segmentation(
                        MultiSTFT(noisy[:, 0], n_fft=hparams["n_fft"]).contiguous(),
                        segment_size=hparams["segment_size"],
                        overlap_ratio=hparams["overlap_ratio"]
                    )[:S].to(self.device)
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.provides("clean_sig", "IBM", "noisy_sig", "enhanced_sig")
    def audio_pipeline(target, noise_list):
        ...
        clean_spec = np.abs(librosa.core.stft(clean_sig, n_fft=1024, hop_length=256)) ** 2
        yield clean_spec
        ...
        scaled_noise_spec = np.abs(librosa.core.stft(scaled_noise, n_fft=1024, hop_length=256)) ** 2
        IBM = (clean_spec > scaled_noise_spec).astype(np.float32)
        yield IBM

        noisy_spec = np.abs(librosa.core.stft(clean_sig + scaled_noise, n_fft=1024, hop_length=256)) ** 2
        yield noisy_spec
        ...
        noisy_enhanced = np.abs(librosa.core.stft(noisy_enhanced, n_fft=1024, hop_length=256)) ** 2
        yield noisy_enhanced
    ...
    for dataset in ["train", "valid"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            output_keys={"id", "noisy_sig", "enhanced_sig", "clean_sig", "IBM"}
...
if __name__ == "__main__":
    ...
    elif hparams["model_name"] == "SB_ver2":
        from model_SB_ver2 import SB_ver2
        model = SB_ver2(
