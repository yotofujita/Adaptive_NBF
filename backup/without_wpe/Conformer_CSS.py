    ...
    def __init__(self, input_dim, kernel_size, dropout_rate, causal=False):
        ...
        if causal:
            self.dw_conv_1d = nn.Conv1d(
                input_dim, input_dim, kernel_size, 1, padding=(kernel_size - 1), groups=input_dim
            )
        else:
            self.dw_conv_1d = nn.Conv1d(
                input_dim, input_dim, kernel_size, 1, padding=(kernel_size - 1) // 2, groups=input_dim
            )
...
class ConformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.encoders = torch.nn.Sequential(
            *[
                EncoderLayer(attention_dim, attention_heads, linear_units, kernel_size, dropout_rate, causal=causal)
    ...
    def __init__(
        ...
        if stats_file is not None:
            ...
            self.input_scale = torch.from_numpy(
                numpy.tile(
                    numpy.expand_dims(1 / numpy.sqrt(stats["variance"].astype(numpy.float32)), axis=0), (1, 1, 1)
                )
            )
            ...
            self.input_bias = torch.zeros(1, 1, n_freq)
            self.input_scale = torch.ones(1, 1, n_freq)
            self.input_bias = nn.Parameter(self.input_bias, requires_grad=False)
            self.input_scale = nn.Parameter(self.input_scale, requires_grad=False)

        # Conformer Encoders
        self.conformer = ConformerEncoder(n_freq, **conformer_conf)
