...
import numpy as np
import torch
from Lightning_CSS_VAD import Lightning_CSS_VAD
...
class MultichannelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        mixture = self.mixture_list[idx]  # M T F
        n_mic, n_time, n_freq = mixture.shape

        feature = torch.zeros([n_time, n_freq, 4 * n_mic - 2])
        feature[..., 0] = torch.log(torch.abs(mixture).mean(axis=0))[None]

        elev, azim = self.angle_list[idx]
        feature[..., 1] = torch.log(torch.abs(torch.einsum("fm, mtf -> tf", self.SV_EAFM[elev, azim].conj(), mixture)))

        IPD = mixture[1:] / (mixture[0, None] + 1e-6)  # M-1 x T x F
        IPD /= torch.abs(IPD) + 1e-6
        feature[..., 2 : 2 * n_mic] = torch.view_as_real(IPD.permute(1, 2, 0)).reshape(n_time, n_freq, -1)

        IPD_SV = self.SV_EAFM[elev, azim, :, 1:] / (self.SV_EAFM[elev, azim, :, 0, None] + 1e-6)  # F x M-1
        IPD_SV /= torch.abs(IPD_SV) + 1e-6
        feature[..., 2 * n_mic :] = torch.view_as_real(IPD_SV).reshape(n_freq, -1)[None]

        feature = feature.reshape(n_time, -1)

        return feature, self.image_list[idx], mixture
...
class LightningDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.train_dataset = MultichannelDataset(window_len=48128, step="train")
        self.valid_dataset = MultichannelDataset(window_len=48128, step="valid")
...
if __name__ == "__main__":
    ...
    parser = Lightning_CSS_VAD.add_model_specific_args(parser, model_name=args_tmp.model_name)
    # --max_epochs, --terminate_on_nan, --auto_lr_find
    parser = pl.Trainer.add_argparse_args(parser)
    args, _ = parser.parse_known_args()

    save_root_dir = "/n/work3/sekiguch/data_for_paper/IROS2022/CSS_VAD/"
    ...
    dm = LightningDataModule(batch_size=16, num_workers=2)
    model = Lightning_CSS_VAD(save_hparam=True, **vars(args))
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        monitor="valid_loss", save_last=True, dirpath=ckpt_dir, every_n_epochs=5
    )
