...
def main(device, idx, BF="MVDR"):
    ...
    if BF == "MVDR":
        save_root_dir = "/n/work3/sekiguch/data_for_paper/IROS2022/DNN_MVDR_WPE_result/test/"
    else:
        raise ValueError
    ...
    for setting in settings:
        ...
        for finetune_dir in glob(
            f"{ckpt_root_dir}/{model_name}-BF={use_BF}-SV={use_SV}-DAN={use_DAN}-VAD={use_VAD}/lightning_logs/version_0/checkpoints/incremental-n_epoch=*-intv_minute={tuning_data_interval_minute}-minute={tuning_data_minute}-ratio={tuning_data_ratio}*"
        ):
