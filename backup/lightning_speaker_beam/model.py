...
class MaskNet(nn.Module):
    ...
    def forward(self, enhanced, spk_emb):
        ...
        x = self.conv_list(x) # [B, F, T]

        return x


class lightning_SB(LightningModule):
    """
    ...
    def __init__(
        ...
        super(lightning_SB, self).__init__()

        self.n_freq = n_freq
        self.z_dim = z_dim
        self.model_name = "lightning_SB"
        self.lr = lr
    ...
    def forward(self, noisy_pwr_spec, enhanced_pwr_spec, clean_pwr_spec, spk_label):
        # spk_label = B
        B, F, T = noisy_pwr_spec.shape
        clean_spk_emb = self.speaker_net(torch.log1p(clean_pwr_spec), None)

        if B > 1:
            clean_spk_emb_list = []
            start_idx = 0
            for i in range(B):
                if (i == B-1) or spk_label[i] != spk_label[i+1]:
        ...
        spk_emb = self.speaker_net(torch.log1p(enhanced_pwr_spec), clean_spk_emb)

        est_mask = self.mask_net(torch.log1p(enhanced_pwr_spec), spk_emb)
        clean_est_mask = self.mask_net(torch.log1p(enhanced_pwr_spec), clean_spk_emb)

        return est_mask, clean_est_mask, spk_emb, clean_spk_emb


    def training_step(self, batch, batch_idx):
        est_mask, clean_est_mask, spk_emb, clean_spk_emb = \
            self.forward(batch.noisy, batch.enhanced, batch.clean, batch.spk_label)
        bce_logit = torch.nn.BCEWithLogitsLoss()
        print("shape : ", est_mask.shape, batch.IBM.shape, clean_est_mask.shape)
        print("dtype : ", est_mask.dtype, batch.IBM.dtype, clean_est_mask.dtype)
        loss1 = bce_logit(est_mask, batch.IBM)
        loss2 = bce_logit(clean_est_mask, batch.IBM)
        loss3 = F.mse_loss(spk_emb, clean_spk_emb)
    ...
    def estimate_mask_segment(self, noisy_pwr_spec_FT, enhanced_pwr_spec_FT, spk_emb_prev):
        spk_emb = self.speaker_net(torch.log1p(enhanced_pwr_spec_FT.unsqueeze(0)), spk_emb_prev)

        mask = torch.sigmoid(self.mask_net(
            torch.log1p(enhanced_pwr_spec_FT.unsqueeze(0)),
            spk_emb
        ).squeeze())
        return mask, spk_emb
...
if __name__ == "__main__":
    ...
    net = SB_orig()
    est_pwr_spec, clean_est_pwr_spec, spk_emb, clean_spk_emb = net.forward(x, y, y, label)
