...
def separate(
    ...
    with torch.no_grad():
        ...
        for b in range(mixture_BMTF.shape[0]):
            mixture_BMTF[b] = torch.as_tensor(wpe.step(cp.asarray(mixture_BMTF[b].permute(2, 1, 0)))).permute(2, 1, 0).to(device)
...
def main(device, idx, BF="MVDR"):
    ...
    if BF == "MVDR":
        save_root_dir = "/n/work3/sekiguch/data_for_paper/IROS2022/WPE_FINETUNED_DNN_MVDR_result/test/"
    else:
        raise ValueError
    ...
    tuning_data_minute, tuning_data_interval_minute = [
        # [3, 3],
        # [6, 3],
        [12, 3],
    ][idx]
    ...
    for setting in settings:
        ...
        for finetune_dir in glob(
            f"{ckpt_root_dir}/{model_name}-BF={use_BF}-SV={use_SV}-DAN={use_DAN}-VAD={use_VAD}/lightning_logs/version_0/checkpoints/wpe_incremental-n_epoch=*-intv_minute={tuning_data_interval_minute}-minute={tuning_data_minute}-ratio={tuning_data_ratio}*"
        ):

            save_dir = (
                f"{save_root_dir}/{model_name}-BF={use_BF}-SV={use_SV}-DAN={use_DAN}-VAD={use_VAD}/"
                + finetune_dir.split("/")[-1]
            )
            os.system(f"mkdir -p {save_dir}")

            ckpt_list = glob(f"{finetune_dir}/checkpoints/epoch=*.ckpt")
            ...
            for ckpt in ckpt_list:
                ...
                for fname in glob(f"{data_root_dir}/test_mixture*SNR=10.wav"):
                    ...
                    sep = separate(
                        model, SV_EAFM[elev_idx, azim_idx], azim_idx, fname, device, insert_first=True, BF=BF
                    )
