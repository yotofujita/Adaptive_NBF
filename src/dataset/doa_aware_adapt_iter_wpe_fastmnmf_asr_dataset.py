...
from src.modules.iter_wpe_fastmnmf import IterWPEFastMNMF


class LightningDataModule(BaseDataModule):
    def __init__(self, batch_size, num_workers, json_path, manifest_path, n_srcs=4, total_s=60, n_gpus=-1, size=-1, **kwargs):
    ...
    def setup(self, stage=None):
        ...
        if stage == "test":
            if self.n_gpus < 2:
                self.test_dataset = TestDataset(self.json_path, size=self.size)
            else:
                print("Not supporting multiple GPUs.")
                assert False
...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, json_path, manifest_path, duration=6.0, sr=16000, total_s=60, n_srcs=2, size=-1):
        assert total_s <= 240
        ...
        # apply fastmnmf 
        print("Apply iterative WPE and FastMNMF to obtain pseudo target speech.")
        model = IterWPEFastMNMF(n_srcs=n_srcs).to("cuda")
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, json_path, duration=6.0, sr=16000, size=-1):
        self.size = size
        ...
        self.asr_target = "TWO MONSTERS ONLY" + data["text"].split("TWO MONSTERS ONLY")[1]
