...
class MaskBasedWPEWMPDR(BaseModule):
    def __init__(self, mask_estimator1, mask_estimator2, delay=3, tap=5, threshold=-10, lr=1e-3, skip_nan_grad=False, **kwargs):
        super().__init__(lr, skip_nan_grad)

        self.mask_estimator1 = instantiate(mask_estimator1)
        self.mask_estimator2 = instantiate(mask_estimator2)
    ...
    def step(self, batch):
        ...
        # Mask estimation
        mask1 = self.mask_estimator1(mix_stft, batch)  # [B, T, F]

        # Dereverberation
        mix_stft_dereverberated = self.dereverberate(mix_stft, mask1, delay=self.delay, tap=self.tap)  # [B, F, T]
    
        # Mask estimation
        mask2 = self.mask_estimator1(mix_stft_dereverberated, batch)  # [B, T, F]
    
        # Beamforming
        sep_stft = self.beamform(mix_stft_dereverberated, mask2)  # [B, F, T]

        # Inverse STFT
        sep = istft(sep_stft)  # [B, S]
        
        return sep, sep_stft, mask1, mask2
    
    @staticmethod
    def dereverberate(mix_stft, mask, delay=3, tap=5, eps=1e-4):
    ...
    def beamform(mix_stft, mask, ref_channel=0, eps=1e-4):
        ...
        R = contract("bkft,blft->bklf", mix_stft / (sigma + eps), mix_stft.conj())  # [B, M, M, F]
...
if __name__ == "__main__":
    ...
    device = "cuda"
    module = MaskBasedWPEWMPDR(cfg1, cfg2).to(device).to(torch.float64)
